...
class LoginController extends Am_Controller_Auth
{
    ...
    {
        if ($this->isAjax()) {
			//$url = $this->getOkUrl().'?access_token='.base64_encode($_SESSION['amember_auth']['user']['user_id']);
			$url = 'http://app-gorilla.com/members/'.'?access_token='.base64_encode($_SESSION['amember_auth']['user']['user_id']);
			return $this->ajaxResponse(array('ok' => true, 'url' => $url));
