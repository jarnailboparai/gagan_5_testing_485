 ...
 */
  ...
  public function output()
  {
    ...
    if(copy($this->_tmp, $this->_output)){
    	//echo $this->_tmp.">>>". $this->_output; 
    	//$image = "<img src='%s' ></img>  <br/>";
    	
    	//printf($image,$this->_output);
    	//printf($image,$this->_tmp);
    	//die;
    	return true;
    }else{
    	return false;
  ...
  public function vignetteoldF($color_1 = 'none', $color_2 = 'black', $crop_factor = 1.5)
  {
  	$crop_x = floor($this->_width * $crop_factor);
  	$crop_y = floor($this->_height * $crop_factor);
  	$this->execute("convert
  	...
  	-fill {$color_1} -colorize 100%
  	-gravity center -crop {$this->_width}x{$this->_height}+0+0 +repage )
  	-compose multiply -flatten
  	{$this->_tmp}");
  }
  
  public function vignetteoldS($color_1 = 'none', $color_2 = 'black', $crop_factor = 1.5)
  {
  	$crop_x = floor($this->_width * $crop_factor);
  	$crop_y = floor($this->_height * $crop_factor);
  	$this->execute("convert
  ...
  public function filterToaster()
  {
    ...
    //$this->vignette('AtominTangerine', 'none');
    //$this->vignette('AtominTangerine', 'none');
    //$this->vignetteoldS('none', 'LavenderBlush3');
    //$this->vignetteoldF('#ff9966', 'none');
  }
