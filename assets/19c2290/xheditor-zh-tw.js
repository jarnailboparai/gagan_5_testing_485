...
var arrFontname=['細明體','黑體','楷體','隸書','幼圓','微軟雅黑','Arial','Arial Narrow','Arial Black','Comic Sans MS','Courier New','System','Times New Roman','Tahoma','Verdana'];
...
var htmlPastetext='<div>使用鍵盤快捷鍵(Ctrl+V)把內容粘貼到方框裡，按 確定</div><div><textarea id="xhEdtPastetextValue" wrap="soft" spellcheck="false" style="width:300px;height:100px;" /></div><div style="text-align:right;"><input type="button" id="xhEdtSave" value="確定" /></div>';
var htmlLink='<div>鏈接地址: <input type="text" id="xhEdtLinkHref" value="http://" class="text" /></div><div>打開方式: <select id="xhEdtLinkTarget"><option selected="selected" value="">默認</option><option value="_blank">新窗口</option><option value="_self">當前窗口</option><option value="_parent">父窗口</option></select></div><div style="text-align:right;"><input type="button" id="xhEdtSave" value="確定" /></div>';
var htmlImg='<div>圖片地址：<input type="text" id="xhEdtImgSrc" value="http://" class="text" /></div><div>替換文本：<input type="text" id="xhEdtImgAlt" /></div><div>對齊方式：<select id="xhEdtImgAlign"><option selected="selected" value="">默認</option><option value="left">左對齊</option><option value="right">右對齊</option><option value="top">頂端</option><option value="middle">居中</option><option value="baseline">基線</option><option value="bottom">底邊</option></select></div><div>寬度高度：<input type="text" id="xhEdtImgWidth" style="width:40px;" /> x <input type="text" id="xhEdtImgHeight" style="width:40px;" /></div><div>邊框大小：<input type="text" id="xhEdtImgBorder" style="width:40px;" /></div><div>水平間距：<input type="text" id="xhEdtImgHspace" style="width:40px;" /> 垂直間距：<input type="text" id="xhEdtImgVspace" style="width:40px;" /></div><div style="text-align:right;"><input type="button" id="xhEdtSave" value="確定" /></div>';
var htmlFlash='<div>動畫地址：<input type="text" id="xhEdtFlashSrc" value="http://" class="text" /></div><div>寬度高度：<input type="text" id="xhEdtFlashWidth" style="width:40px;" value="480" /> x <input type="text" id="xhEdtFlashHeight" style="width:40px;" value="400" /></div><div style="text-align:right;"><input type="button" id="xhEdtSave" value="確定" /></div>';
var htmlMedia='<div>視頻地址：<input type="text" id="xhEdtMediaSrc" value="http://" class="text" /></div><div>寬度高度：<input type="text" id="xhEdtMediaWidth" style="width:40px;" value="480" /> x <input type="text" id="xhEdtMediaHeight" style="width:40px;" value="400" /></div><div style="text-align:right;"><input type="button" id="xhEdtSave" value="確定" /></div>';
var htmlTable='<div>行數列數：<input type="text" id="xhEdtTableRows" style="width:40px;" value="3" /> x <input type="text" id="xhEdtTableColumns" style="width:40px;" value="2" /></div><div>標題單元：<select id="xhEdtTableHeaders"><option selected="selected" value="">無</option><option value="row">第一行</option><option value="col">第一列</option><option value="both">第一行和第一列</option></select></div><div>寬度高度：<input type="text" id="xhEdtTableWidth" style="width:40px;" value="200" /> x <input type="text" id="xhEdtTableHeight" style="width:40px;" value="" /></div><div>邊框大小：<input type="text" id="xhEdtTableBorder" style="width:40px;" value="1" /></div><div>表格間距：<input type="text" id="xhEdtTableCellSpacing" style="width:40px;" value="1" /> 表格填充：<input type="text" id="xhEdtTableCellPadding" style="width:40px;" value="1" /></div><div>對齊方式：<select id="xhEdtTableAlign"><option selected="selected" value="">默認</option><option value="left">左對齊</option><option value="center">居中</option><option value="right">右對齊</option></select></div><div>表格標題：<input type="text" id="xhEdtTableCaption" /></div><div style="text-align:right;"><input type="button" id="xhEdtSave" value="確定" /></div>';
var htmlAbout='<div style="width:200px;word-wrap:break-word;word-break:break-all;"><p><span style="font-size:20px;color:#1997DF;">xhEditor</span><br />版本：v0.9.9 build 20091123</p><p>xhEditor是一個基於jQuery開發的跨平台開源迷你XHTML編輯器組件。</p><p><a href="http://xheditor.com/" target="_blank">http://xheditor.com/</a></p></div>';
...
var arrTools={GStart:{},GEnd:{},Separator:{},Cut:{t:'剪切 (Ctrl+X)'},Copy:{t:'複製 (Ctrl+C)'},Paste:{t:'粘貼 (Ctrl+V)'},Pastetext:{t:'粘貼文本'},Blocktag:{t:'段落標籤'},Fontface:{t:'字體'},FontSize:{t:'字號'},Bold:{t:'加粗 (Ctrl+B)',s:'Ctrl+B'},Italic:{t:'斜體 (Ctrl+I)',s:'Ctrl+I'},Underline:{t:'下劃線 (Ctrl+U)',s:'Ctrl+U'},Strikethrough:{t:'中劃線 (Ctrl+S)',s:'Ctrl+S'},FontColor:{t:'字體顏色'},BackColor:{t:'背景顏色'},Removeformat:{t:'刪除文字格式'},Align:{t:'對齊'},List:{t:'列表'},Outdent:{t:'減少縮進 (Shift+Tab)',s:'Shift+Tab'},Indent:{t:'增加縮進 (Tab)',s:'Tab'},Link:{t:'超鏈接'},Unlink:{t:'取消超鏈接'},Img:{t:'圖片'},Flash:{t:'Flash動畫'},Media:{t:'視頻'},Emot:{t:'表情'},Table:{t:'表格'},Source:{t:'源代碼'},Preview:{t:'預覽'},Fullscreen:{t:'全屏編輯 (Esc)',s:'Esc'},About:{t:'關於 xhEditor'}};
var toolsThemes={
...
$.xheditor=function(textarea,options)
{
	...
	{
		var s1='<span class="upload"><input type="text" style="visibility:hidden;" /><input type="button" value="上傳" class="btn" />',jUpload;
		...
		{//系統默認ajax上傳
			...
			jFile.change(function(){
				...
				if(sFile!='')
				{
					...
					{
						var modal=_this.showModal('文件上傳','<div style="margin:22px 0;text-align:center;line-height:30px;">文件上傳中，請稍候……<br /><img src="'+skinPath+'img/loading.gif"></div>',320,150);
