...
*/			//die("retun re");
	...
	public function actionBuildPhoneGapAppMy($id = null) 
	{
		...
		if(!$param->pg_appid)
			...
			$app_info = $phone->uploadAppMy($param->git_repo_url, $param->title, 'remote_repo',null,$param->pg_appid);
	...
	public function actionCurlMyPhone($id)
	{
		...
		{
			echo $id;
/*			$cmd = "sh ./gittest/shellscript/checking.sh ";
			var_dump($cmd);
			$mm = shell_exec($cmd);
		 */	
			var_dump($mm);
	...
	private function moveFolder($repo)
	{
		if (isset($repo))
		{
			echo $cmd = "sh ./gittest/shellscript/gitcp.sh {$repo}";
			$mm =shell_exec($cmd);
			return true;
		}
		return true;
	...
	private function commitRepo($repo)
	{
		if (isset($repo))
		{
			echo $cmd = "sudo sh ./gittest/shellscript/gituploadnew.sh ".$repo;
			$mm = shell_exec($cmd);
			return true;
		}
		//Yii::app()->end();
