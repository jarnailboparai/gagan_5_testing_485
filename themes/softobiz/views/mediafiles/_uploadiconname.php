...
<!--  		<div id="queue" class="align-center"></div>  -->
		...
		<div class="align-center uploads">
			...
			<a  id="upload_file" class="btn btn-warning" href="javascript:$('#file_upload').uploadifive('upload')">Upload Icons</a>
		</div>
	...
	<script type="text/javascript">
			...
			$('#file_upload').uploadifive({
				...
				'formData'         : {
					   ...
					   'token'     : '<?php echo md5('unique_salt' . time());?>'
				                     ...
				                     *
				                     */?>
                 <?php //if($app_icon_new == 1) { ?>									 
				'fileType'         : 'image/png',	
				 <?php //} ?>				 
				'queueID'          : 'queueOpen',
				'uploadScript'     : '<?php echo CHtml::normalizeUrl(array('mediafiles/uploadiconnew','app_icon_new'=>$app_icon_new))?>',
				 ...
				 'onAddQueueItem' : function(file,queue) {
					    /* var file_type = file.type;
						 if(file_type != 'image/png')
						 {
							...
							var id = $("#queueOpen .uploadifive-queue-item:last").attr('id');
							jQuery('#'+id+ " div:first ").addClass('remove').text("Please select only PNG Image");
		...
		{
			jQuery('#myModalAppIconName').modal('hide');
			jQuery('.modal-backdrop').remove();
			jQuery('#icon_select').removeData("modal");
			<?php if($app_icon_new == 2) { ?>
			jQuery('#icon_select').modal({remote: "<?php echo CHtml::normalizeUrl(array('mediafiles/index_appicon','module_id'=>$module_id,'layout'=>1,'app_icon_new'=>$app_icon_new))?>"});
			<?php }else { ?>
			jQuery('#icon_select').modal({remote: "<?php echo CHtml::normalizeUrl(array('mediafiles/index_appicon','module_id'=>$module_id,'layout'=>1,'app_icon_new'=>$app_icon_new))?>"});
			<?php } ?>
			//flagloader = false;
